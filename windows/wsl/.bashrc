...
# https://github.com/microsoft/WSL/issues/4106#issuecomment-673011368
powershell.exe -C "Stop-Process -Name vcxsrv -EA SilentlyContinue"
"/mnt/c/Program Files/VcXsrv/xlaunch.exe" -run "C:\Users\shortpoet\Documents\xlaunch\config.xlaunch"
export DISPLAY=$(cat /etc/resolv.conf | grep nameserver | awk '{print $2}'):0
HOST=$(ip address show dev eth0 | awk -F '[ /]+' '/inet / { print $3 }')
WSLENV="$WSLENV:DISPLAY" "/mnt/c/Program Files/VcXsrv/xhost.exe" "$HOST" >/dev/null

# https://github.com/microsoft/WSL/issues/4106#issuecomment-643775895
# HOST=$(ip address show dev eth0 | awk -F '[ /]+' '/inet / { print $3 }')
